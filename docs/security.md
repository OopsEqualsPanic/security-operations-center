...
| ``efi=disable_early_pci_dma`` | Prevents malicious PCI devices from performing Direct Memory Access attacks during the boot process, and ensures that the busmaster bit is cleared on all PCI bridges before the operating system takes control. |
